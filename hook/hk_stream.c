...
void dt_udp_ingress(const unsigned char *buf, size_t n) {
    ...
    if (buf[3] == DU_C2S) {
        ...
        if (8 + il2 + 2 > (int)n) return;
        {
            ...
            const unsigned char *pl = buf + 10 + il2;
            size_t rl = n - 10 - (size_t)il2;
            if (!dt_hosted_udp_in(gp2, buf + 8, il2, cport2, pl, rl))
                dt_direct_udp_in(gp2, buf + 8, il2, pl, rl);
        }
        return;
    }
...
#ifdef LINUX_BUILD
            dt_rand_seed();
            dt_rand_mix((unsigned)dt_now_ms());
#else
            dt_rand_seed();
            dt_rand_mix((unsigned)GetTickCount());
#endif
            g_node = dt_rand() | 1;
            slp_set_node(g_node);
        }
