...
void dt_acc_learn_fd(long long fd, const struct sockaddr_in *peer, int listen_port) {
    ...
    for (int i = 0; i < DT_MAXHOST; i++) {
        ...
        /* lport is the bridge ephemeral accept() reports as the peer.
 ...
 * listen VPORT. On a machine hosting one of our nodes the listener is
 * aliased beneath its vport (two nodes, one real stack), so the
